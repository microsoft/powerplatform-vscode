...
export class PortalWebView {
    ...
    public static checkDocumentIsHTML(showWarning: boolean): boolean {
        ...
        if (!result && showWarning) {
            vscode.window.showInformationMessage("The current editor doesn't a file which can be previewed.");
        }
        return result;
    }
	...
	public static createOrShow(context: vscode.ExtensionContext) {
        ...
        const isHtml = this.checkDocumentIsHTML(true);
        if (!isHtml) {
            return;
        }
    ...
    private getFileName(): string {
        ...
        const fileTitle = '(Preview) ' + filePath.substring(filePath.lastIndexOf('\\') + 1);
        return fileTitle;
    }
	...
	private addStyles(webview: vscode.Webview,html: string): string {
		const uri = vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders[0].uri;

		// Add bootstrap.min.css
		let url =webview.asWebviewUri(vscode.Uri.joinPath(uri as vscode.Uri, 'web-files', 'bootstrap.min.css'));
		let bootstrap: string = `<link href="${url}" rel="stylesheet" />`;

		// Add theme.css
		url = webview.asWebviewUri(vscode.Uri.joinPath(uri as vscode.Uri, 'web-files', 'theme.css'));
		let theme: string = `<link href="${url}" rel="stylesheet" />`;
	...
	private fixLinks(webview: vscode.Webview, html: string): string {
		...
		const uri = vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders[0].uri;
		const BaseURL = webview.asWebviewUri(vscode.Uri.joinPath(uri as vscode.Uri, 'web-files'));

		// update img src value with base url of web-files folder
		// html = html.replace(/<img([^>]*)\ssrc=(['"])(\/[^\2*([^\2\s<]+)\2/gi, "<img$1 src=$2" + BaseURL + "$3$2");
		var regex = /<img([^>]*)\ssrc=(['"])(\/[^\2*([^\2<]*(png|jpg|jpeg|svg|gif|PNG|JPG|JPEG|SVG|GIF|bmp|BMP))/g
