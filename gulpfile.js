...
const isOfficialBuild =
    argv.isOfficialBuild && argv.isOfficialBuild.toLowerCase() == "true";
const isPreviewBuild =
    argv.isPreviewBuild && argv.isPreviewBuild.toLowerCase() == "true";
...
function setTelemetryTarget() {
    const telemetryConfigurationSource =
        isOfficialBuild && !isPreviewBuild
...
async function packageVsix() {
    ...
    const setPackageInfo = async function (pkgOptions) {
        ...
        gulp.src("README.md")
            .pipe(
                replace(
                    pkgOptions.readmeReplacementTarget,
                    pkgOptions.readmeHeader
                )
            )
            .pipe(gulp.dest("./"));
    };
    ...
    await setPackageInfo(
        isPreviewBuild ? previewPackageOptions : standardPackageOptions
    );
...
async function translationsExport() {
    ...
    return gulp
        .src("./loc/translations-export/vscode-powerplatform.xlf")
        .pipe(replace("&apos;", "'"))
        .pipe(replace("&#10;", "\n"))
