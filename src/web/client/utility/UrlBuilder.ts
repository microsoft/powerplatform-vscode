...
import { httpMethod, MULTI_ENTITY_URL_KEY, pathParamToSchema, portalSchemaVersion, powerpagecomponents, SINGLE_ENTITY_URL_KEY } from "../common/constants";
...
export function getRequestURL(dataverseOrgUrl: string, entity: string, entityId: string, entitiesSchemaMap: Map<string, Map<string, string>>, method: string, isSingleEntity: boolean): string {
    ...
    switch (method) {
        case httpMethod.GET:
            if (entitiesSchemaMap.get(pathParamToSchema.get(entity) as string)?.get('_fetchQueryParameters'))
    ...
    if (dataSourcePropertiesMap.get(portalSchemaVersion) === 'V2') {
        parameterizedUrlTemplate += `({entityId})`;
        entity = powerpagecomponents;
    }

    parameterizedUrlTemplate = parameterizedUrlTemplate.replace('{dataverseOrgUrl}', dataverseOrgUrl).replace('{entity}', entity)
