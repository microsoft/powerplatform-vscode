...
import { columnExtension, CONTENT_PAGES, NO_CONTENT, EMPTY_FILE_NAME, DEFAULT_LANGUAGE_CODE, entityFolder, FILE_NAME_FIELD, MULTI_ENTITY_URL_KEY, ORG_URL, pathParamToSchema, WEBSITE_ID, WEBSITE_NAME, telemetryEventNames, OLD_SCHEMA_NAME } from './constants';
...
export async function fetchData(accessToken: string, entity: string, entityId: string, queryParamsMap: Map<string, string>, entitiesSchemaMap: Map<string, Map<string, string>>, languageIdCodeMap: Map<string, string>, portalFs: PortalsFS, websiteIdToLanguage: Map<string, string>, schema: string) {
    ...
    try {
        ...
        if (schema === OLD_SCHEMA_NAME) {
            ...
            if (data.value?.length >= 0) {
                for (let counter = 0; counter < data.value.length; counter++) {
                    createContentFiles(data[counter], entity, queryParamsMap, entitiesSchemaMap, languageIdCodeMap, portalFs, dataverseOrgUrl, accessToken, entityId, websiteIdToLanguage, schema);
                }
            } else {
                createContentFiles(data, entity, queryParamsMap, entitiesSchemaMap, languageIdCodeMap, portalFs, dataverseOrgUrl, accessToken, entityId, websiteIdToLanguage, schema);
            }
        } else {
            url = queryParamsMap.get(SINGLE_ENTITY_URL_KEY);
            const requestUrl = getRequestURLForSingleEntity(dataverseOrgUrl, '', '', url as string, entitiesSchemaMap, 'GET');
            sendAPITelemetry(url as string);
            requestSentAtTime = new Date().getTime();
            const response = await fetch(requestUrl, {
                headers: getHeader(accessToken),
            });
            if (!response.ok) {
                vscode.window.showErrorMessage(ERRORS.BACKEND_ERROR);
                sendAPIFailureTelemetry(url as string, new Date().getTime() - requestSentAtTime, response.statusText);
                throw new Error(response.statusText);
            }
            sendAPISuccessTelemetry(url as string, new Date().getTime() - requestSentAtTime);
            const data = await response.json();
            if (data.value?.length >= 0) {
                for (let counter = 0; counter < data.value.length; counter++) {
                    createContentFiles(data[counter], entity, queryParamsMap, entitiesSchemaMap, languageIdCodeMap, portalFs, dataverseOrgUrl, accessToken, entityId, websiteIdToLanguage, schema);
        ...
        sendAPIFailureTelemetry(url as string, new Date().getTime() - requestSentAtTime, authError);
    }
}
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function createContentFiles(result: any, entity: string, queryParamsMap: Map<string, string>, entitiesSchemaMap: Map<string, Map<string, string>>, languageIdCodeMap: Map<string, string>, portalsFS: PortalsFS, dataverseOrgUrl: string, accessToken: string, entityId: string, websiteIdToLanguage: Map<string, string>, schema: string) {
    ...
    if (schema === OLD_SCHEMA_NAME) {
        ...
        let fileName = EMPTY_FILE_NAME;
        const fetchedFileName = result.name;
        if (fetchedFileName)
            fileName = result[fetchedFileName].toLowerCase();
        if (fileName === EMPTY_FILE_NAME) {
            showErrorDialog(ERRORS.FILE_NAME_NOT_SET, ERRORS.SERVICE_ERROR);
        ...
        portalsFS.createDirectory(vscode.Uri.parse(`${PORTALS_URI_SCHEME}:/${portalFolderName}/${subUri}/${fileName}/`, true));
        portalsFS.createDirectory(vscode.Uri.parse(`${PORTALS_URI_SCHEME}:/${portalFolderName}/${subUri}/${fileName}/${entityFolder.get(CONTENT_PAGES)}/`, true));
        const value = result.Summary ? result.Summary : NO_CONTENT;
        saveDataMap = createVirtualFile(portalsFS, fileName, languageCode, value, columnExtension.get('summary') as string, subUri, entityId, 'summary', entity, portalFolderName);
        vscode.window.showTextDocument(vscode.Uri.parse(`${PORTALS_URI_SCHEME}:/${portalFolderName}/${subUri}/${fileName}/${entityFolder.get(CONTENT_PAGES)}/${fileName}.${languageCode}.${columnExtension.get('summary') as string}`));
    }
    registerSaveProvider(accessToken, portalsFS, dataverseOrgUrl, saveDataMap, schema);
}
...
export async function getDataFromDataVerse(accessToken: string, entity: string, entityId: string, queryParamMap: Map<string, string>, entitiesSchemaMap: Map<string, Map<string, string>>, languageIdCodeMap: Map<string, string>, portalFs: PortalsFS, websiteIdToLanguage: Map<string, string>, schema: string) {
    vscode.window.showInformationMessage(INFO.FETCH_FILE);
    await fetchData(accessToken, entity, entityId, queryParamMap, entitiesSchemaMap, languageIdCodeMap, portalFs, websiteIdToLanguage, schema);
}
