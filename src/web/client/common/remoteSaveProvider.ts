...
export function registerSaveProvider(
    ...
    useBase64Encoding: boolean, schema: string
) {
    vscode.workspace.onDidSaveTextDocument(async (document) => {
        if (document?.uri?.fsPath) {
            ...
            const patchRequestUrl = getRequestURL(dataVerseOrgUrl,
                ...
                httpMethod.PATCH,
                true);
            await saveData(accessToken, patchRequestUrl, document.uri, saveDataMap, stringDecodedValue, schema, portalsFS);
        }
    });
}
...
export async function saveData(
    ...
    fileUri: vscode.Uri,
    saveDataMap: Map<string, SaveEntityDetails>,
    value: string, schema: string, portalFs: PortalsFS
) {
    let requestBody = '';
    ...
    if (requestBody) {
        ...
        if (schema === NEW_SCHEMA_NAME) {
            {
                vscode.window.showInformationMessage('saving file: ' + fileUri);
                const olddata: JSON | undefined = saveDataMap.get(fileUri.fsPath)?.getData
                const newFileData = portalFs.readFile(fileUri)
                const value = new TextDecoder().decode(newFileData);
                const newJsonData = JSON.parse(olddata as unknown as string);
                const key = 'copy';
