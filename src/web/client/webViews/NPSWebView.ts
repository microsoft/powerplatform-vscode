...
import * as cancel from '../assets/cancel.svg';
...
export class NPSWebView  {
    ...
    WebExtensionContext.setNPSEligibility(true);
       const mainJs = this.extensionResourceUrl('media','main.mjs');
        return `<!DOCTYPE html>
            <html lang="en">
            <head>
                <meta charset="UTF-8">
            ...
            <body>
                ...
                <script type="module" id="abc" data-eligibility="${cancel}" nonce="${nonce}" src="${mainJs}"></script>
            </body>
            </html>`;
    }
  ...
  private static applyCustomStyles() {
      ...
      if (crossButtonDiv) {
        ...
        console.log("close button"+crossButtonDiv)
    ...
    public static createOrShow(extensionUri: vscode.Uri): NPSWebView {
        const webview = vscode.window.createWebviewPanel(
            ...
            },
            {
                enableScripts:true,
                localResourceRoots: [
                  vscode.Uri.joinPath(extensionUri, 'media','main.mjs')
                ]
            }
        );
