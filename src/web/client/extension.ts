...
let userViewProvider: TreeWebView;

export function activate(context: vscode.ExtensionContext): void {
    // setup telemetry

    userViewProvider = new TreeWebView();
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand(
            ...
            async (args) => {
                ...
                WebExtensionContext.setVscodeWorkspaceState(
    ...
    processCreateCopresenceWorkerInstance(context);
...
export function processWalkthroughFirstRunExperience(
        ...
        vscode.workspace.onWillSaveTextDocument(async (e) => {
            ...
            if (vscode.window.activeTextEditor === undefined) {
                ...
                if (fileData?.entityId && fileData.attributePath) {
                    ...
                    fileContent = convertContentToString(
                        fileContent,
                        fileData.encodeAsBase64 as boolean
                    );
                    updateEntityColumnContent(
                        fileData?.entityId,
...
export function processCreateCopresenceWorkerInstance(
    context: vscode.ExtensionContext
) {
    // Create a web worker instance
    const copresenceInstance = vscode.Uri.joinPath(
        context.extensionUri,
        "dist/web/CopresenceWorkerService.worker.js"
    );

    const copresenceInstanceURL = new URL(copresenceInstance.toString());

    fetch(copresenceInstanceURL)
        ...
        .then((workerScript) => {
            ...
            copresenceWorker.onmessage = (event) => {
                ...
                WebExtensionContext.setContainerId(data.containerId);
