...
class MyTreeItem extends vscode.TreeItem {
  ...
  private getIconPath(item: IItem): { light: string, dark: string } | vscode.ThemeIcon {
    const basePath = path.join(__dirname, '..', 'src', 'common', 'portal_fileicons', 'icons');
  ...
  vscode.commands.registerCommand('extension.openFile', async (item: IItem) => {
    try {
      if (item.path) {
        ...
        if (pathString.endsWith('.html') || pathString.endsWith('.css') || pathString.endsWith('.js') || pathString.endsWith('.json')) {
