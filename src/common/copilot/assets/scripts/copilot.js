...
(function script() {
    ...
    function createWalkthroughDiv() {
        ...
        walkthrough.innerHTML = `<h4 class="walkthrough-title">${sanitizeMessage(copilotStrings.GETTING_STARTED_MESSAGE)}</h4><a href="#" class="walkthrough-content" tabindex="0" aria-label="${sanitizeMessage(copilotStrings.LEARN_MORE_MESSAGE)}">${bookIconSvg}<span id="walk-text">${sanitizeMessage(copilotStrings.LEARN_MORE_MESSAGE)}</span></a>`; // CodeQL [SM04949] no user input is used, therefore these need not be sanitized.
        return walkthrough;
    }
