...
(function script() {
    ...
    function createSuggestedPromptDiv() {
        ...
        suggestedPrompt.innerHTML = `<p class="suggested-title">${sanitizeMessage(copilotStrings.SUGGESTIONS_MESSAGE)}</p><a href='#' class="suggested-prompt" tabindex="0" aria-label="${sanitizeMessage(formPrompt)}"><span class="icon-container">${starIconSvg}</span>${sanitizeMessage(formPrompt)}</a><br><a href='#' class="suggested-prompt" tabindex="0" aria-label="${sanitizeMessage(webApiPrompt)}"><span class="icon-container">${starIconSvg}</span>${sanitizeMessage(webApiPrompt)}</a><br><a href='#' class="suggested-prompt" tabindex="0" aria-label="${sanitizeMessage(listPrompt)}"><span class="icon-container">${starIconSvg}</span>${sanitizeMessage(listPrompt)}</a>`; // CodeQL [SM03712] no user input is used, therefore these need not be sanitized.
    ...
    function createWalkthroughDiv() {
        ...
        walkthrough.innerHTML = `<h4 class="walkthrough-title">${sanitizeMessage(copilotStrings.GETTING_STARTED_MESSAGE)}</h4><a href="#" class="walkthrough-content" tabindex="0" aria-label="${sanitizeMessage(copilotStrings.LEARN_MORE_MESSAGE)}">${bookIconSvg}<span id="walk-text">${sanitizeMessage(copilotStrings.LEARN_MORE_MESSAGE)}</span></a>`; // CodeQL [SM03712] no user input is used, therefore these need not be sanitized.
        return walkthrough;
    }
